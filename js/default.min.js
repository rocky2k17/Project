  ...
  function libDefine() {
    ...
    app.findParent = function(htmlElement, name) {
      if (htmlElement != null && name!=null) {
       ...
       // debugger;
        while (1) {
          if (al.tagName == "html" || al.tagName == "HTML") break;
    ...
    app.init = function() {
      ...
      for (var i = 0, len = items.length; i < len; i++) {
        if (items[i].getAttribute("href") == "" ||items[i].getAttribute("href") == "#") items[i].href = "javascript:";
        if (app.hasClass(items[i], "form") && items[i].getAttribute("data-validate") == "true")app.validate(items[i]);
        ...
        if (app.hasClass(items[i], "dropdown")) {
          document.dropdown.push(items[i]);
        }
      ...
      app.addEvent("click", body, function(e) {
        ...
        if (app.hasClass(target, "switch-off")) {
        ...
        if (app.hasClass(target, "btn") && !app.hasClass(target, "active") && app.findParent(target,"group-btn")[0]) {
        ...
        // Ok
        if (app.findParent(target, "dropbtn")[0] == 1) {
          var parent = app.findParent(target, "dropdown");
          if (parent[0] == 1 && app.hasClass(parent[1], "hover"))
            app.removeClass(parent[1], "hover");
          else {
            for (var i = 0; i < document.dropdown.length; i++) {
              app.removeClass(document.dropdown[i], "hover");
            }
        ...
        // Ok
        var x= app.findParent(target, "dropdown-content");
          if(typeof x !== 'undefined' && x.length > 0 && x[0]==1){
         // console.log("yes");
          ...
          if (parent[0] == 1 && parent[1].getAttribute("data-name") != null) {
        ...
        // Ok
          ...
          if (app.findParent(target, "modal")[0]) {
            app.toggleClass(app.findParent(target, "modal")[1], "show", "hide");
          } else if (app.findParent(target, "alert")[0] == 1) {
            app.toggleClass(app.findParent(target, "alert")[1], "show", "hide");
          }
        }
        // Ok
        else if (app.findParent(target, "navbar-icon")[0]) {
        ...
        if (app.findParent(target,"accordion-head")[0]) {
